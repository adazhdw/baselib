...
import com.adazhdw.ktlib.KtLib.getApp
import java.util.*
...
class SPUtil {
    ...
    companion object {
        ...
        @JvmOverloads
        @JvmStatic
        fun getInstance(spName: String = "", mode: Int = Context.MODE_PRIVATE): SPUtil {
            var spName1 = spName
            if (isSpace(spName1)) {
                spName1 = "spUtils"
            }
            var spUtils = SP_UTILS_MAP[spName1]
            if (spUtils == null) {
                synchronized(SPUtil::class.java) {
                    spUtils = SP_UTILS_MAP[spName1]
                    if (spUtils == null) {
                        spUtils = SPUtil(spName1, mode)
                        SP_UTILS_MAP[spName1] = spUtils!!
                    }
                }
            }
            return spUtils!!
