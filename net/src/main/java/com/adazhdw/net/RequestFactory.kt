...
class RequestFactory(builder: Builder) {
    ...
    inline fun <reified T : Any> toClazz(): Call<T> {
        return InternalAdapters.parse<T, Call<T>>(object : TypeRef<Call<T>>() {}.type, net, this)
    }

    inline fun <reified T : Any, reified R : Any> toClazz(typeRef: TypeRef<R>): R {
        return InternalAdapters.parse<T, R>(typeRef.type, net, this)
    }

    fun download(): Call<okhttp3.ResponseBody> {
        return InternalAdapters.parse<okhttp3.ResponseBody, Call<okhttp3.ResponseBody>>(object : TypeRef<Call<okhttp3.ResponseBody>>() {}.type, net, this)
    }
    ...
    class Builder(val net: Net) {
        ...
        inline fun <reified T : Any> toClazz(): Call<T> {
            return build().toClazz<T>()
        }

        inline fun <reified T : Any, reified R : Any> toObject(): R {
            return build().toClazz<T, R>(object : TypeRef<R>() {})
        }
